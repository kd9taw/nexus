...
| Fading penalty | large (coherent) | ~3.7 dB *(sim)* | 10+ dB (coherent-class) | — | — |
...
- **Sensitivity.** FT8 (~−21 dB) is the most sensitive single-shot mode; FT1 (~−15 dB) sits roughly where FT4 does (~−17.5 dB), give or take. FT1 gives up ~3 dB to FT8 in exchange for a *much* shorter cycle and the IR-HARQ path. This is directly analogous to how **FT4 itself trades ~3.5 dB of sensitivity vs FT8 for roughly half the cycle time** — FT1 just pushes that lever further and adds incremental redundancy. *(Note: some JS8 secondary sources quote FT8 theory near −24 dB; that is **not** the standard operational figure — use ~−21 dB.)*
...
- **FT8/FT4 tier is Phase 2.** The `Tier::Ft8` variant exists and the FT8/FT4 DSP sources compile into `libft1`, but **no end-to-end FT8/FT4 decode pipeline is wired.** Do not treat that tier as functional yet.
