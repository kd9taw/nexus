//! End-to-end FFI round trip through libft1:
...
fn encode_decode_roundtrip_high_snr() {
    ...
    assert_eq!(tones.len(), ft1::NN, "FT1 frame should be 99 symbols");
    ...
    let mut buf = ft1::frame_align(&wave); // dt0 = 0, zero-padded to NMAX
    let mut rng = Awgn::new(12345);
    for s in buf.iter_mut() {
        *s = sig * *s + rng.gaussian();
    }

    let decoded = ft1::decode_rt(&buf, F0, SNR_DB);
    assert!(
        decoded.ok(),
...
fn encode_is_deterministic_and_99_symbols() {
    let a = ft1::encode(MSG);
    let b = ft1::encode(MSG);
    assert_eq!(a, b);
    assert_eq!(a.len(), 99);
    assert!(
