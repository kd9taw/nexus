//! Safe Rust wrapper over `libft1`, the FT1 4-CPM turbo weak-signal modem.
//!
...
//! reworking that state in `libft1`.
//!
//! # Timing
...
//! frequency/time search), which is a planned `libft1` ABI addition.

pub use ft1_sys::{FT1_MSG91 as MSG91, FT1_NMAX as NMAX, FT1_NN as NN};
...
// modes (FT1/FT8/FT4/DX1) that link the single libft1 — defined in ft1-sys so
// ft1/ft8/ft4 contend on one lock (see ft1_sys::MODEM_LOCK).
...
pub fn gen_wave(itone: &[i32], fsample: f32, f0: f32) -> Vec<f32> {
    ...
    let _guard = MODEM_LOCK.lock().unwrap();
    unsafe {
        ft1_sys::ft1_gen_wave(
            itone.as_ptr(),
            itone.len() as i32,
            ft1_sys::FT1_NSPS_NUM,
            ft1_sys::FT1_NSPS_DEN,
            fsample,
            f0,
            wave.as_mut_ptr(),
            &mut nwave,
...
#[allow(clippy::too_many_arguments)] // mirrors the libft1 C ABI surface
pub fn decode_frame(
    iwave: &[i16],
    nfa: i32,
    nfb: i32,
    ...
    let mut out = vec![ft1_sys::Ft1DecodeT::default(); MAX_DECODES];

    let n = {
        let _guard = MODEM_LOCK.lock().unwrap();
        unsafe {
            ft1_sys::ft1_decode_frame(
                iwave.as_ptr(),
                nfa,
                nfb,
                ndepth,
...
pub mod dx1 {
    ...
    pub fn decode_band(wave: &[f32], f_lo: f32, f_hi: f32, fsample: f32) -> Vec<Decode> {
        ...
        let mut out = vec![ft1_sys::Dx1DecodeT::default(); MAX_DX1_DECODES];
        let n = {
            let _guard = MODEM_LOCK.lock().unwrap();
            unsafe {
                ft1_sys::dx1_decode_band(
                    wave.as_ptr(),
                    wave.len() as i32,
                    f_lo,
                    f_hi,
...
mod dx1_tests {
    ...
    fn dx1_encode_decode_roundtrip() {
        ...
        let wave = dx1::encode_wave(msg, dx1::F0, SAMPLE_RATE);
        assert_eq!(wave.len(), dx1::frame_len(), "DX1 wave is one frame long");

        // Drop the waveform into a full capture window at a non-zero offset so
        // the chirp sync has to find it.
        let cap = dx1::capture_len();
        let mut window = vec![0f32; cap];
        let off = 12_000; // 1 s in
        window[off..off + wave.len()].copy_from_slice(&wave);

        let d =
            dx1::decode(&window, dx1::F0, SAMPLE_RATE).expect("DX1 decodes its own clean signal");
        assert_eq!(d.message, msg);
        assert_eq!(d.freq, dx1::F0);
    }
    ...
    fn dx1_decode_band_multi_signal() {
        ...
        for (msg, f0, off) in sigs {
            let w = dx1::encode_wave(msg, f0, SAMPLE_RATE);
        ...
        let decodes = dx1::decode_band(&window, 200.0, 2900.0, SAMPLE_RATE);
...
mod harq_encode_tests {
    ...
    // Costas sync arrays per redundancy version (must match genft1.f90).
